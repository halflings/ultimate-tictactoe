...
% 6. Avoid going on a grid where you need one extra move to win
...
cellWeight(N, M, J, W) :- baseWeight(M, W1), emptyGridWeight(M, J, W2), avoidSpoiledWin(N, J, W3), winWeight(N, M, J, W4),
    spoilWinWeight(N, M, J, W5), avoidWinWeight(N, M, J, W6), avoidWonWeight(N, M, J, W7),
  	W is W1 + W2 + W3 + W4 + W5 + W6 + W7, !.
...
avoidSpoiledWin(N, J, 8) :- not(isWinningMove(N, _, J)). 
avoidSpoiledWin(_, _, 0).
